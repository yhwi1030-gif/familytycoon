...
export const ProfileSelection: React.FC<ProfileSelectionProps> = ({ onSelect }) => {
  ...
  const handleOnboardingComplete = async (data: any) => {
    ...
    const newProfile: Profile = {
      ...
      level: 1,
      exp: 0,
      gold: onboardingRole === 'child' ? 0 : 1000,
      stress: 0,
      style: data.style,
            ...
            {profiles.map((p) => (
              ...
              >
                ...
                <div className="w-16 h-16 rounded-2xl bg-indigo-950/30 group-hover:bg-indigo-950/50 border border-slate-800 flex items-center justify-center overflow-hidden transition select-none mb-4">
                  {p.role === 'parent' ? (
                    p.avatar === '🧙‍♀️' ? (
                      <img src="/mom.png?v=3" alt="Guildmaster Mama" className="w-full h-full object-contain scale-85" />
                    ) : p.avatar === '🧙‍♂️' ? (
                      <img src="/dad.png?v=3" alt="Sub Guildmaster Papa" className="w-full h-full object-contain scale-85" />
                    ) : (
