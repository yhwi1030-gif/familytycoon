...
const mapQuestToDB = (q: any) => {
  ...
  if (q.feedback) meta.feedback = q.feedback;
...
const mapQuestFromDB = (q: any): Quest => {
  ...
  if (rawDueTime.includes('||')) {
    ...
    try {
      ...
      if (!parsed.rewardStats && !parsed.sweetCheered && !parsed.feedback) {
        rewardStats = parsed;
      }
    } catch (e) {
...
export const api = {
  ...
  // 체험용 기본 mock 데이터 강제 생성 (새로고침 우회)
  setupDemoData: async (): Promise<void> => {
    if (typeof window !== 'undefined') {
      ...
      if (isSupabaseConfigured) {
        try {
          ...
          console.error("Supabase setupDemoData error:", err);
        }
      }
    }
  }
